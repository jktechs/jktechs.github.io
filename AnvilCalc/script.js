...
function makeLevelTree(level, makeElm){
    if(level === 0)
        return undefined
    let el = makeElm(makeLevelTree(level-1, makeElm), makeLevelTree(level-1, makeElm));
    return el;
}
function makeTree(Ileafs, makeElm){// leafs = 2^(l-1)    ln2(leafs)+1
    let leafs = [...Ileafs]
    let level = Math.floor(Math.log2(leafs.length)+1)
    let toAdd = leafs.length - Math.pow(2, level-1)
    let tree = makeLevelTree(level, makeElm)
    let elms = []
    for(let i = 0;i<toAdd*2;i++)
        elms.push(leafs.pop())
    let func = function (branch, clevel) {
        if(branch.left !== undefined){
            arguments.callee(branch.left,clevel+1)
            arguments.callee(branch.right,clevel+1)
            if(elms.length === 0 && clevel === level-1){
                if(branch.left.left === undefined)
                    branch.left = leafs.pop();
                if(branch.right.left === undefined)
                    branch.right = leafs.pop();
            }
        } else {
            if(elms.length !== 0){
                branch.left = elms.pop();
                branch.right = elms.pop();
            }
        }
    }
    func(tree,1)
    ...
    for (let items of permutations(itemOptions)) {
        ...
        BFS2(tree, branch => {
            ...
            if (
                ...
                sacrifice.book === undefined ||
                (target.book && !sacrifice.book)
            ) {
            ...
            for (let ench of sacrifice.ench) {
                enchCost +=
                    ench[1] *
                    (sacrifice.book ? Math.max(Math.floor(ench[0] / 2), 1) : ench[0])
            }
            branch.total =
                target.total +
                sacrifice.total +
                Math.pow(2, target.used) +
                Math.pow(2, sacrifice.used) -
                2 +
                enchCost
        })
...
function addBox(name) {
    ...
    e.onchange = v => {
        if (v.target.checked) items.push(div.innerText)
        else items.splice(items.indexOf(div.innerText), 1)
    }
}
